#' Sandwich Variance Estimator for Internal Poisson (Log-Linear) Regression Calibration
#'
#' \code{sandwich_estimator_in_poisson()} computes robust (sandwich) standard errors
#' for regression calibration in Poisson log-linear regression using internal replicate
...
#'   typically from \code{\link{reg_calibration_in_poisson}}.
...
#' @param Y Non-negative integer count outcome vector of length \eqn{n}.
#' @param v12star Calibration submatrix (between-person exposure block).
#' @param beta.fit2 Numeric vector of Poisson regression coefficients from the
...
#' @param fit2 The fitted \code{glm} object returned by \code{reg_calibration_in_poisson()}.
...
#'         p-values, rate ratios (RR), and 95\% confidence intervals on the
#'         original scale.}
#' }
#'
#' @details
...
#'   \item \strong{Outcome model}: fits a Poisson regression and applies the
...
#' r <- rep(2, 50)
#'
#' # (In practice, run reg_calibration_in_poisson() first to obtain xhat, fit2, etc.)
#' # Here we show a simplified call with mock objects:
#' # sandwich_estimator_in_poisson(xhat, zbar.std, z.std, r, Y,
...
sandwich_estimator_in_poisson = function(xhat,zbar,z.std,r,Y,v12star,beta.fit2,W.std = NULL,sigma,
  ...
  if(is.null(W.std)){
    ...
    d = as.matrix(p %*% t(beta.fit2[2:(t+1)])) / n
    ...
    p = as.vector(exp(beta.fit2 %*% t(cbind(1,xhat,W.std))))
    sigmaz.inv = solve(sigmazstar)
