...
#'
#' \code{reg_calibration_ex_poisson()} corrects for classical additive
#' measurement error in Poisson regression with a log link using data from
#' an external reliability study. It implements regression calibration by
#' estimating between- and within-subject variance components from replicate
#' measurements and then fitting a Poisson regression to the calibrated
#' exposures. A robust (sandwich) variance estimator is used for valid inference.
#'
...
#'         coefficients, robust (sandwich) standard errors, z-values,
#'         p-values, exponentiated coefficients (rate ratios), and 95\%
...
#'   \item Calibrate each main-study measurement \eqn{Z_i} to
#'         \eqn{E[X_i \mid Z_i]} using the calibration matrix
#'         \eqn{\Sigma_X \Sigma_Z^{-1}}.
#'   \item Fit a Poisson regression model with log link using the calibrated
#'         exposures \eqn{X_i^\text{hat}}. Exponentiated coefficients are
#'         interpreted as rate ratios.
#' }
#'
#' @examples
#' set.seed(123)
